...
population_flows <- xap.read_table("population_data")
...
server <- function(input, output, session) {
  ...
  track_radians <- reactive({
    create_track_radians(radians = radians(), names = as.character(region_totals_out$region_orig), approx_points = 400)
  })
  
  seq_df <- reactive({
    create_seq_df(names = as.character(region_totals_out$region_orig), lengths = total_flow(), radians = radians())
  })
